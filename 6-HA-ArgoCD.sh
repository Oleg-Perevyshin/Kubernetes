#!/bin/bash
# Сделать файл исполняемым на машине мастера chmod +x 5-HA-ArgoCD.sh;
...
# ----------------------------------------------------------------------------------------------- #
ssh -i "/root/.ssh/$CERT_NAME" "root@${NODES[s1]}" bash <<ARGOCD
  set -euo pipefail
...
EOF
  ...
  echo -e "${GREEN}  Логин: admin | Пароль: ${ARGOCD_PASSWORD}${NC}"
  echo -e "${GREEN}  Логин: admin | Пароль: \$(kubectl -n argocd-system get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d)${NC}"
