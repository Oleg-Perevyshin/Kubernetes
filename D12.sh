...
qm create "$VMID" --name "Debian-12-Clean" --ostype l26 \
...
cat << 'EOF' > /var/lib/vz/snippets/debian-clean.yaml
#cloud-config
package_update: true
packages:
  - qemu-guest-agent
...
qm set "$VMID" --cicustom "vendor=local:snippets/debian-clean.yaml"
