...
ssh -q -t -i "$HOME/.ssh/$CERT_NAME" "$USER@${NODES[server]}" sudo bash <<EOF
  ...
  echo -e "${GREEN}  Применяем cert-manager.crds.yaml${NC}";
  ...
  helm upgrade -i cert-manager jetstack/cert-manager --namespace cert-manager --create-namespace --wait --timeout 180m || {
    echo -e "${RED}  Ошибка установки Cert-Manager, установка прервана${NC}"; exit 1;
  }
  ...
  helm upgrade -i rancher rancher-latest/rancher \
    ...
    --wait --timeout 180m || {
      echo -e "${RED}  Ошибка установки Rancher, установка прервана${NC}"; exit 1;
  }
...
echo -e "${GREEN}${NC}"
echo -e "${GREEN}Rancher установлен и доступен по адресу: https://${RANCHER_HOST}${NC}"
echo -e "${GREEN}${NC}"
