...
echo -e "${GREEN}  Директория $HOME/.kube подготовлена${NC}"
...
# Проверяем конфигурационный файл SSH
...
echo -e "${GREEN}  Конфигурационный файл SSH проверен${NC}"
...
# Добавляем SSH ключи ко всем узлам
...
for host in "${NODES[@]}"; do
  sshpass -p "$PASSWORD" ssh-copy-id -o StrictHostKeyChecking=no -i "$CERT_PATH" "$USER@$host" >/dev/null 2>&1 || {
    echo -e "${RED}  Ошибка при передаче ключа на $host, установка прервана${NC}"
    exit 1
  }
done
echo -e "${GREEN}  SSH ключ добавлен ко всем узлам кластера${NC}"

# Подготавливаем все узлы
for newnode in "${ALL_CLUSTER_ITEMS[@]}"; do
  # shellcheck disable=SC2087
  ssh -q -t -i "$HOME/.ssh/$CERT_NAME" "$USER@$newnode" sudo bash <<EOF
    echo -e "${GREEN}  Подготовка узла $newnode${NC}";
    set -e;
